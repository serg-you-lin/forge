...
# 2. solo le parti che passano un filtro (callable(part) -> bool)
big = forge.to_dxf(result, doc, filter_part=lambda p: p.outer.polygon.area > 1000)
path = os.path.join(OUTDIR, f"{base}_big_only.dxf")
big.saveas(path)
print(f"filter_part area -> {path}")
