...
from .geometry import entity_to_polygon, get_representative_point, copy_entity
from .layers import (
...
def split_to_files(
    ...
    for i, part in enumerate(result.parts, start=1):
        ...
        # Il polygon del padre viene dal ForgeResult in memoria —
        # funziona anche per loop LINE+SPLINE senza LWPOLYLINE nel msp
        ...
        # Inizializza tutti i layer forge nel documento figlio.
        # I colori vengono da layers.py — fonte di verità —
        # indipendentemente da quello che le entità hanno addosso.
        for layer_name, color in ALL_FORGE_LAYERS.items():
            new_doc.layers.add(layer_name, color=color)

        # Cerca l'entità outer nel msp — prima cerca su LAYER_OUTER,
        # poi come fallback usa il polygon per trovare entità corrispondenti
        outer_written = False
        ...
        # Fallback: loop con SPLINE — copia le entità originali su LAYER_OUTER
        # (heal() le ha già classificate con il layer corretto)
        ...
        # Inner / Hole — contenuti nel polygon del padre
        ...
        # Entità extra contenute nell'outer
        ...
        # Data injection PRIMA del namer — così _codice è disponibile
        if data_injector is not None:
            try:
                testi = extract_texts_from_msp(new_msp)
...
#         # Crea i layer strutturali nel documento figlio
#         layer_colors = {
#             LAYER_OUTER: COLOR_OUTER,
#             LAYER_INNER: COLOR_INNER,
#             LAYER_HOLE:  COLOR_HOLE,
