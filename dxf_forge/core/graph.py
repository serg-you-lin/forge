...
from .geometry import arc_endpoints, arc_to_bulge
...
def round_point(pt, decimals=1):
    """Arrotonda un punto (x, y) a `decimals` cifre decimali."""
...
def build_node_graph(msp, decimals=1, exclude_layers=None):
    ...
    for entity in msp.query('SPLINE'):
        ...
        if s_pt is not None and e_pt is not None:
            s = round_point(s_pt, decimals)
            e = round_point(e_pt, decimals)
            graph[s].append((entity, e))
            graph[e].append((entity, s))
...
def find_closed_loops(graph):
    ...
    """
    visited_edges = set()  # condiviso — ogni entità entra in un solo loop
    loops = []
    ...
    for start_node in graph:
        for (entity, next_node) in graph[start_node]:
            ...
            for e, rev in chain:
                if e.dxftype() == 'LINE':
                    pts.append((e.dxf.end.x, e.dxf.end.y) if rev
