...
def heal(
    ...
    Passi:
        1. LINE/ARC/SPLINE → grafo → loop → VirtualShape in memoria
        2. Gerarchia padre-figlio → ForgeResult con ForgePart
        3. [opzionale] Scrittura nel msp via _apply_to_msp()
 
    Note:
        - Il file sorgente non viene mai modificato se write_to_msp=False.
        - L'injection di dati custom (data_injector) è responsabilità
          del chiamante, da eseguire sul ForgeResult restituito.
          Vedi split_to_files() per l'uso tipico.
    ...
    if not any([all_lines, all_arcs, all_plines,
                all_circles, all_splines, all_ellipses]):
