...
def show(tag, result):
    p = result.clusters[0]
    print(f"{tag:<24} bending={len(p.features("bending_lines")):<3} engrave={len(p.features("engrave_lines")):<3} "
...
for bl in r_linetype.clusters[0].features("bending_lines")[:5]:
    print(f"   len={bl.length:6.1f}  angle={bl.angle_deg:6.1f}  source={bl.source}")
print(f"   ... {len(r_linetype.clusters[0].features("bending_lines"))} totali")
...
print(f"bending={len(r_color.clusters[0].features("bending_lines"))} qui, non {len(r_linetype.clusters[0].features("bending_lines"))}: "
