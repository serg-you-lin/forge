...
# e il routing _loop_to_virtual_shape.
# """
...
#         # mock start_point / end_point usati da _parse_arc
...
# # ---------------------------------------------------------------------------
# # _loop_to_virtual_shape — routing e risultato finale
# # ---------------------------------------------------------------------------

# class TestLoopToVirtualShape(unittest.TestCase):

#     def test_001_loop_line_ha_has_spline_false(self):
#         vs = _loop_to_virtual_shape(make_square_loop(100.0), 'outer', 1)
#         self.assertFalse(vs.has_spline)

#     def test_002_loop_line_ha_pts_with_bulge(self):
#         vs = _loop_to_virtual_shape(make_square_loop(100.0), 'outer', 1)
#         self.assertGreater(len(vs.pts_with_bulge), 0)

#     def test_003_loop_spline_ha_has_spline_true(self):
#         spline = make_spline_entity([(0,0),(0,50),(0,100)])
#         loop = [
#             (make_edge(spline),                          False),
#             (make_edge(make_line(0, 100, 100, 100)),     False),
...
#     def test_004_loop_spline_ha_pts_with_bulge_vuoto(self):
#         spline = make_spline_entity([(0,0),(0,50),(0,100)])
#         loop = [
#             (make_edge(spline),                          False),
#             (make_edge(make_line(0, 100, 100, 100)),     False),
...
#     def test_005_loop_vuoto_restituisce_none(self):
#         vs = _loop_to_virtual_shape([], 'outer', 1)
#         self.assertIsNone(vs)

#     def test_006_loop_una_line_restituisce_none(self):
...
from forge.core.primitives.virtual import VirtualShape
from forge.model import Edge
from forge.adapters.dxf.virtual_adapter import (
    parse_loop, _loop_to_virtual_shape, DxfWriteContext,
)
...
class TestLoopToVirtualShape(unittest.TestCase):
    ...
    def test_001_loop_line_restituisce_dxf_write_context(self):
        ctx = _loop_to_virtual_shape(make_square_loop(100.0), 'outer', 1)
        self.assertIsInstance(ctx, DxfWriteContext)

    def test_002_ctx_ha_vs_virtual_shape(self):
        ctx = _loop_to_virtual_shape(make_square_loop(100.0), 'outer', 1)
        self.assertIsInstance(ctx.vs, VirtualShape)

    def test_003_loop_line_has_spline_false(self):
        ctx = _loop_to_virtual_shape(make_square_loop(100.0), 'outer', 1)
        self.assertFalse(ctx.vs.has_spline)
    ...
    def test_005_ctx_ha_loop(self):
        ...
        ctx = _loop_to_virtual_shape(loop, 'outer', 1)
    ...
    def test_006_loop_spline_ha_has_spline_true(self):
        ...
        with patch('forge.adapters.dxf.virtual_adapter.spline_to_points',
                   side_effect=lambda e: e._mock_points):
            ctx = _loop_to_virtual_shape(loop, 'outer', 1)
        self.assertTrue(ctx.vs.has_spline)
    ...
    def test_008_loop_vuoto_restituisce_none(self):
        ctx = _loop_to_virtual_shape([], 'outer', 1)
