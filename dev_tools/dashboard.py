...
def plot_healed(ax, result, filename):
    _setup_ax(ax, f"OUTPUT  ·  {filename}")
    legend_seen = {}

    for part in result.parts:

        # outer + inners sono tutti ForgeContour — stessa logica
...
def _add_legend(ax, legend_seen):
    ...
    ax.legend(handles=handles, loc='upper right',
              ...
              labelcolor=C_TEXT, fontsize=10, framealpha=0.92)
...
def plot_metadata(ax, result, filename, tolerance):
    ...
    for i, part in enumerate(result.parts):
        rows.append((f'── PART {i+1}', part.label or '—', C_ACCENT))
        rows.append(('   Area',   f'{part.area:.2f} mm²',  C_TEXT))
        rows.append(('   Holes',  str(len(part.inners)),    C_TEXT))
        bbox = part.bbox
        if bbox:
            w = round(bbox[2] - bbox[0], 2)
            h = round(bbox[3] - bbox[1], 2)
            rows.append(('   BBox', f'{w} × {h} mm', C_TEXT))
        perim = round(part.outer.polygon.exterior.length, 2)
        rows.append(('   Perimeter', f'{perim} mm', C_TEXT))
        rows.append(('', '', C_DIM))
...
def run(input_dxf, tolerance=0.05, output_png=None, special_layers=None):
    ...
    plot_healed(ax_healed,   result,              filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dxf',       default='tests/examples/linee_di_piegatura_interne.dxf')
