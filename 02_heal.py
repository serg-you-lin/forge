...
consegna ForgePart(outer, inners=[ForgeContour...]).
...
result = forge.heal(
    ...
    label="P-DEMO",                # finisce in part.label e nei metadati
    source_file="Linee_piegatura.dxf",
)

print(f"is_valid   : {result.is_valid}")
print(f"parti      : {result.part_count}")
...
for i, part in enumerate(result.parts):
    ...
    print(f"   outer  : role={part.outer.role}  area={part.outer.area:.1f}  "
          f"perimetro={part.outer.polygon.exterior.length:.1f}")
    print(f"   bbox   : {tuple(round(v, 1) for v in part.bbox)}")
    print(f"   inners : {len(part.inners)}  (contorni interni, non ancora tipati come foro)")
    print(f"   holes  : {len(part.holes)}   (sempre 0 dopo heal — li fa detect)")
